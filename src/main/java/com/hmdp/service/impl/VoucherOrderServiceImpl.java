...
public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder> implements IVoucherOrderService {
    ...
    public Result seckillVoucher(Long voucherId) {
        ...
        *//*synchronized (userId.toString().intern()) {
            // 获取代理对象 （事务）
            // 事务,默认是由aop代理对象进行控制，如果我们直接调用createVoucherOrder方法，
            // 那么是调用了当前类VoucherOrderServiceImpl的方法，不是调用代理对象IVoucherOrderService
            // 因此直接调用，事务不会执行
...
//        SimpleRedisLock lock = new SimpleRedisLock("order:" + userId, stringRedisTemplate);
    ...
    // 如果在方法就加悲观锁(synchronized), 那么后面任何用户每次访问都要竞争锁，这样无法实现只对单个用户创建订单进行限制
    @Transactional
    public void createVoucherOrder(VoucherOrder voucherOrder) {
        // 6.1 查询订单
        Long userId = voucherOrder.getUserId();
        // 如果在这里对用户id加锁，可实现只对单个用户创建订单进行限制，但是这样在每次返回后，事务提交之前, 锁就被释放了，
        // 这样其他线程也可以竞争该锁了，这样依然会导致线程安全问题
        /*synchronized (userId.toString().intern()) {*/
